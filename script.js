...
let notaAcustica = 0.60; // pitch
let duracionAcustica = 1.5; // duracion 
let umbral = 0.03;
...
function draw() {
  ...
  if (terminaElSonido) {
    ...
    /*
      ...
      notaActual >= 72 &&
      notaActual <= 74 &&
      gestorI.filtrada > 0.03;

    if (esBorrador) {
      background(255);
      console.log("¡BORRADOR detectado! Canvas limpio");
    }
    */

    let esShhh = 
      duracion < 1.0 &&                  // es corto
      gestorI.filtrada > 0.03 &&         // se escucha algo
      (isNaN(notaActual) || notaActual === 0); // no hay nota
    if (esShhh) {
      background(255);
      console.log("¡SHHH detectado! Canvas limpio");
      return; // salimos para no dibujar figuras
    }
...
function colorAzulClaro(nota, duracion) {
  let r = map(nota, 40, 80, 150, 110);   // CAMBIAR ESTE COLOR POR UNO DECENTE WACHO
